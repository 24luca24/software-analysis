...
public class Query {
    ...
    public boolean queryControlloUser(DataBase db, String user) throws SQLException {
        String s = "";
        boolean bol = true;
        try (ResultSet result = db.submitQuery("SELECT username FROM Utenti WHERE username = '" + user + "'")) {
            if (result == null) {
    ...
    public ArrayList<Playlist> queryRicercaPlaylist(DataBase db, String username) throws SQLException {
        ...
        while (result.next()) {
            Playlist provvisoria = new Playlist("", result.getString(1), result.getString(2));
            listaRicercaPl.add(index, provvisoria);
            index++;
        }
        for (Playlist p : listaRicercaPl) {
            String s = "";
            ResultSet result3 = db3.submitQuery(
                    "SELECT idPlaylist FROM Playlist WHERE nomePlaylist = '" + p.getNome() + "'"
            );
            while (result3.next()) {
            ...
            String provv = "";
            ResultSet result2 = db2.submitQuery(
                    "SELECT idCanzone FROM canzoniPlaylist WHERE idPlaylist = '" + s + "'"
            );
            while (result2.next()) {
                provv += result2.getString(1) + "$";
            }

            // Remove trailing "$" if present
            if (provv.endsWith("$")) {
                provv = provv.substring(0, provv.length() - 1);
            }

            Playlist k = new Playlist(provv, p.getUsername(), p.getNome()); // ✅ full constructor
        ...
        public void queryPopolaDb(DataBase db) throws SQLException {
            ...
            if (n < 1) {
                ...
                try (Connection connection = (Connection) db.getConnection()) {
                    connectionUrl = (HttpURLConnection) percorsoProgramma.openConnection();
                    try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(connectionUrl.getInputStream()))) {
